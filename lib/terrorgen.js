...
/**///
/**/// Notes
/**/// note   - make the directory, alert if exists
Terrorgen.prototype.makeDirectory = function() {
  var self = this

  if (fs.existsSync(self.projectName)) {
    console.log('**WARNING**'.red)
    console.log('Directory /%s already exists.'.red, self.projectName)
    console.log('**ALL FILES AND FOLDERS IN THE LOCATION WILL BE DESTROYED AND REPLACED**'.red)
    console.log('To overwrite the directory please type the project name again.')
    process.stdin.resume()
    process.stdin.setEncoding('utf8')
    process.stdin.on('data', function(data) {
      process.stdin.pause()
      data = data.replace('\n','').replace('\r','')
      if (data === self.projectName) {
        rimraf.sync('./' + self.projectName)
        console.log('./%s has been destroyed. Creating new project.'.green, self.projectName)
        fs.mkdirSync(self.projectName)
        self.generate()
      } else {
        console.log('goodbye')
        self.brokenHeart()
